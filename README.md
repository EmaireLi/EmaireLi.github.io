...
首页按“现有个人介绍 → 文章归档 → 项目记录 → 留言”排列，保留液态玻璃样式和项目卡片结构。

维护原则：

- 公共导航使用“文章 / 项目 / 关于”，沿用 `#blog`、`#projects`、`#about`。
- 编辑器入口放在首页页脚；修改导航时同步现有页面、编辑器导出模板和小红书导入模板。
- 不添加站内路标或泛化板块导语，个人介绍与文章原文保持原样。
- 静态和动态归档仅显示日期、标题和标签；清单中的摘要和全文数据继续供搜索使用。
