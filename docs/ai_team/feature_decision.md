# Feature Decision Index

Current implementation focus: Round 11 Informed Public Guestbook Entry.

## Latest Decision

- Round: 11
- Selected feature: Informed Public Guestbook Entry
- Reason: makes the public publishing consequence clear before a visitor types or submits personal text.
- Detailed decision: [round-11-feature_decision.md](./round-11-feature_decision.md)
- Implementation plan: [implementation_plan.md](./implementation_plan.md)

## Recent Decision History

- [Round 11 - Informed Public Guestbook Entry](./round-11-feature_decision.md)
- [Round 10 - Addressable Archive Filter State](./round-10-feature_decision.md)
- [Round 09 - Static Archive Baseline](./round-09-feature_decision.md)
- [Round 08 - Accessible Mobile Back-to-Top](./round-08-feature_decision.md)
- [Round 07 - Canonical Mobile Search Doorway](./round-07-feature_decision.md)
- [Round 06 - Curated Reading Paths](./round-06-feature_decision.md)
- [Round 05 - Site Pipeline Lens](./round-05-feature_decision.md)
- [Round 04 - Homepage Evidence Routing](./round-04-feature_decision.md)
- [Round 03 - Guided Code Tour](./round-03-feature_decision.md)
- [Round 02 - Project Case Study Pages](./round-02-feature_decision.md)

## Selection Rule

New homepage features should improve the personal website's visual structure, visitor routing, content presentation, interaction quality, or trust. Features centered mainly on external GitHub projects should be documented as supporting evidence, not the main site-design direction.
